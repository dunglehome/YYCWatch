 ...
 *
 * By ppival@ucalgary.ca & dule@ucalgary.ca
 * Originated from https://github.com/kevinodotnet/ottwatch
 */
    ...
    public static function geoOttawaImport($table, $files) {
        ...
        foreach ($files as $f) {
            ...
            foreach ($data->features as $f) {
                if (++$index % 80 == 0) {
                    print " $index/" . count($data->features) . "\n";
                }
                switch ($data->geometryType) {
                    case "esriGeometryPoint":
        ...
        foreach ($datasets as $d) {
            ...
            foreach ($set->resources as $r) {
                ...
                if ($row['id']) {
                    # exists
                    $values['id'] = $row['id'];
                    if ($r->url == 'http://opl-bpo.ca/en/rss-feeds' || $r->url == 'http://octranspo1.com/developers/register' || $r->url == 'http://biblioottawalibrary.ca/branches.xml' || $r->url == 'http://app06.ottawa.ca/online_services/opendata/data/spotlight/events.xml' || $r->url == 'http://app06.ottawa.ca/online_services/opendata/data/spotlight/venues.xml'
                    ) {
...
#						}
    ...
    public static function callData($path) {
        ...
        # $json = `wget -qO - "http://data.calgary.ca/$path"`;
        # var_dump($json);
    ...
    public static function doList() {
        ...
        <div class="row-fluid">
            ...
            <div class="span9">
                <p class="lead">
                    <b>data.calgary.ca</b> is Calgary's open data portal. As data is added/updated

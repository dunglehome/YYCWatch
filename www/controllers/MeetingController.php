...
class MeetingController {
    ...
    ");
        foreach ($rows as $m) {
            ...
            print "Agenda\n";
            $url_en = "http://agendaminutes.calgary.ca/sirepub/mtgviewer.aspx?meetid={$m['meetid']}&doctype=AGENDA";
            $url_fr = "http://agendaminutes.calgary.ca/sirepub/mtgviewer.aspx?meetid={$m['meetid']}&doctype=AGENDA2";
            print "<a href=\"$url_en\">English</a>\n";
            print "<a href=\"$url_fr\">French</a>\n";
            print "<br/>\n";

            print "Minutes\n";
            $url_en = "http://agendaminutes.calgary.ca/sirepub/mtgviewer.aspx?meetid={$m['meetid']}&doctype=MINUTES";
            $url_fr = "http://agendaminutes.calgary.ca/sirepub/mtgviewer.aspx?meetid={$m['meetid']}&doctype=MINUTES2";
            print "<a href=\"$url_en\">English</a>\n";
            print "<a href=\"$url_fr\">French</a>\n";
            print "<br/>\n";
            print "<br/>\n";
            ...
            print "<ul>\n";
            foreach ($items as $i) {
                $item_url = "http://agendaminutes.calgary.ca/sirepub/item.aspx?itemid={$i['itemid']}";
                print "<li>";
                ...
                if (count($files) > 0) {
                    print "<ul>\n";
                    foreach ($files as $f) {
                        $file_url = "http://agendaminutes.calgary.ca/sirepub/view.aspx?cabinet=published_meetings&fileid={$f['fileid']}";
                        print "<li>\n";
    ...
    static public function votesMember($name) {
      ...
      ", array('name' => $name));
        ...
        $totalYesNo = 0;
        foreach ($votes as $v) {
            if ($v['vote'] == 'y' && $v['passed'] == 0) {
                $againstMajorityTotal ++;
                continue;
            }
            if ($v['vote'] == 'n' && $v['passed'] == 1) {
                $againstMajorityTotal ++;
                continue;
            }
            if ($v['vote'] == 'a') {
    ...
    static public function getVideoStart($id) {
        ...
        $url = "http://agendaminutes.calgary.ca/sirepub/mtgviewer.aspx?meetid={$meetid}&doctype=AGENDA";
    ...
    # added "120" filter because sometimes Ottawa.ca puts in meetings that
      ...
      ", array('last' => $last));
        ...
        # get the data
        $curl = 'http://agendaminutes.calgary.ca/sirepub/view.aspx?cabinet=published_meetings&fileid=' . $fileid;
        ...
        $url = "http://agendaminutes.calgary.ca/sirepub/$data";
    ...
    static public function getDocumentUrl($meetid, $doctype) {
        return "http://agendaminutes.calgary.ca/sirepub/agview.aspx?agviewmeetid={$meetid}&agviewdoctype={$doctype}";
    }

    static public function getItemUrl($itemid) {
        return "http://agendaminutes.calgary.ca/sirepub/agdocs.aspx?doctype=agenda&itemid=" . $itemid;
    }
...
#    }
#    foreach ($files as $f) {
#      $url = "http://agendaminutes.calgary.ca/sirepub/view.aspx?cabinet=published_meetings&fileid={$f['fileid']}";
...
#  }
    ...
    static public function meetingDetails($category, $meetid, $itemid) {
      ...
      where p.itemid in (select id from item where meetingid = :meetingid) ", array("meetingid" => $m['id']));
        ...
        <div class="row-fluid">
            ...
            <div class="span4">
                ...
                print "<a href=\"http://agendaminutes.calgary.ca/sirepub/mtgviewer.aspx?meetid=" . $meetid . "&doctype=AGENDA\"><i class=\"icon-share\"></i> View on Calgary.ca</a>";
                print "</small>";
                ?>
                ...
                <div id="agendanav" style="overflow:scroll; height: 620px;">
                    ...
                    foreach ($items as $i) {
                        ...
                        print "<a href=\"http://agendaminutes.calgary.ca/sirepub/item.aspx?itemid={$i['itemid']}\"><i class=\"icon-share-alt\"></i></a>";
                        print "<br/>\n";
                            ...
                            <table class="table table-bordered table-hover table-condensed" style="width: 100%;">
                                ...
                                foreach ($votes as $vote) {
                                    ...
                                    if ($lastitemtitle != $itemtitle) {
                                        // http://agendaminutes.calgary.ca/sirepub/agdocs.aspx?doctype=minutes&itemid=301229
                                        ?>
                                        <tr>
        ...
        ?>
                                                ...
                                                <div class="tab-pane fade" id="tabdelegation">
                                                    ...
                                                    <div style="padding: 10px; padding-top: 0px; overflow: scroll; height: 600px;">
                                                        ...
                                                        $cmtmailto = "mailto:" . implode(",", $emails) .
      ...
      order by
        starttime desc ", array('category' => $category));
                                                    foreach ($rows as $r) {
                                                        $mtgurl = htmlspecialchars("http://agendaminutes.calgary.ca/sirepub/mtgviewer.aspx?meetid={$r['meetid']}&doctype");
    ...
    getDatabase()->execute(" delete from item where meetingid = :id ", array('id' => $id));
	          ...
	          ", array(
                                                ...
                                                $origitemids = array();
                                                foreach ($orig_items as $i) {
                                                    $origitemids[] = $i['itemid'];
                                                }
                                                $nowitemids = array();
                                                foreach ($now_items as $i) {
                                                    $nowitemids[] = $i['itemid'];
                                                }
